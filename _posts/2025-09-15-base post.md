# Application of Cubical Homology on the example of PepsiCo Lab Potato Chips Quality Control Dataset

## Introduction



y increase $\tau$, connected components appear and disappear, and so do $1$-dimensional holes. We say that a topological cavity (connected component or hole) is **born** at the threshold value (time $t$) when it first appears, and it **dies** when it either merges with another topological cavity of the same kind (connected component or hole, respectively) or disappears entirely. This process can be captured using **Persistent Barcodes**, where each bar represents a topological cavity from the moment it is born to the moment it dies.  
